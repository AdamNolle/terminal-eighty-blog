...
router.post('/passkey/register/start', async (req, res) => {
    ...
    db.prepare('INSERT INTO challenges (id, challenge, type, user_id) VALUES (?, ?, ?, ?)').run(
        challengeId, options.challenge, 'registration', user.id
    );
...
router.post('/passkey/register/finish', async (req, res) => {
    ...
    const challengeRow = db.prepare('SELECT * FROM challenges WHERE id = ? AND type = ?').get(challengeId, 'registration');
    ...
    try {
        ...
        if (verification.verified && verification.registrationInfo) {
            ...
            db.prepare(`INSERT INTO passkeys (id, user_id, credential_id, public_key, counter, transports)
                VALUES (?, ?, ?, ?, ?, ?)`).run(
                passkeyId,
...
router.post('/passkey/login/finish', async (req, res) => {
    ...
    const challengeRow = db.prepare('SELECT * FROM challenges WHERE id = ? AND type = ?').get(challengeId, 'authentication');
    if (!challengeRow) return res.status(400).json({ error: 'Invalid challenge' });
